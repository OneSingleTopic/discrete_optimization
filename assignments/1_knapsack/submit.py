...
def load_metadata(metadata_file_name="_coursera"):
    ...
    try:
        with open(metadata_file_name, "r") as metadata_file:
            ...
            if len(name) <= 0:
                print(
                    "Empty assignment name in _coursera file: %s"
                    % metadata_file_name
                )
                quit()
...
def output(input_file, solver_file):
    ...
    try:
        ...
        print(
            "the solve_it(input_data) method from solver.py raised an exception"
        )
        print(
...
def login_dialog(
    assignment_key, results, credentials_file_location="_credentials"
):
    """
