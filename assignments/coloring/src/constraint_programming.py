def graph_search(node_count, edges):
    ...
    edges_adjacents = [
        (
            ...
            [
                ...
                for elem in list(filter(lambda x: (x[0]==node or x[1]==node), edges))
            ]
        )
    ...
    while any(s is None for s in solution):
        ...
        while to_visit:
            ...
            for color in colors: 
                if all(
                    solution[neighbor] is None or solution[neighbor]!=color for neighbor in neighbors
                ):
